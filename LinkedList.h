...
// Deletes a course by ID (needed so ADD_COURSE can be undone)
inline bool removeCourseByID(CourseList* ls, std::string id) {
    if (ls->n == 0) return false;

    CourseElement* curr = ls->head;
    CourseElement* prev = nullptr;

    while (curr != nullptr) {
        if (curr->data.courseID == id) {
            if (curr == ls->head) {
                ls->head = curr->next;
