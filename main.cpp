...
int main() {
    ...
    // Give the undo stack real references so undo() can actually
    // change the data, not just print a message about it.
    ...
    do
    {
        ...
        case 3:
        {
            ...
            // Capture the OLD gpa before overwriting it, so undo can restore it
            StudentElement* target = searchStudentByID(studentList, id);
            if (target != nullptr)
            {
                double oldGpa = target->data.gpa;
                updateStudentGPA(studentList, id, gpa);
                undoStack.pushUpdateGPA(id, oldGpa);
        ...
        case 4:
        {
            ...
            // Snapshot the student BEFORE deleting, so undo can restore them
            StudentElement* target = searchStudentByID(studentList, id);
            if (target != nullptr)
            {
                Student snapshot = target->data;
                deleteStudentByID(studentList, id);
                undoStack.pushDeleteStudent(snapshot);
        ...
        case 12:
        {
            ...
            undoStack.pushQueueEnqueue(sid, cid);
        ...
        case 13:
        {
            ...
            if (removeFromQueueByID(waitQueue, sid, cid)) {
                ...
                undoStack.pushQueueDequeue(sid, cid);
            } else {
